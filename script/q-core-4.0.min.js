...
,getWindow:function(bK){return qx.dom.Node.getWindow(qx.module.Traversing.__getElementFromArgument(bK));}
...
,getDocument:function(bN){return qx.dom.Node.getDocument(qx.module.Traversing.__getElementFromArgument(bN));}
,getNodeName:function(bO){return qx.dom.Node.getName(qx.module.Traversing.__getElementFromArgument(bO));}
,getNodeText:function(bP){return qx.dom.Node.getText(qx.module.Traversing.__getElementFromArgument(bP));}
,isBlockNode:function(bQ){return qx.dom.Node.isBlockNode(qx.module.Traversing.__getElementFromArgument(bQ));}
,equalNodes:function(ca,cc){ca=qx.module.Traversing.__getNodeFromArgument(ca);cc=qx.module.Traversing.__getNodeFromArgument(cc);if(!ca||!cc){return false;}
...
;}
;if(bY){var bR=qx.module.Traversing.__getAttributes(ca);var cb=qx.module.Traversing.__getAttributes(cc);for(var bT in bR){if(bR[bT]!==cb[bT]){return false;}
;}
;}
...
;var ba=qx.event.handler.PointerCore.MSPOINTER_TO_POINTER_MAPPING[X.type]||X.type;var Y=qx.bom.Event.getTarget(X);var W=X.type;var V=new qx.event.type.dom.Pointer(ba,X);this._fireEvent(V,ba,Y);if(W!==ba&&Y.parentNode){V=new qx.event.type.dom.Pointer(W,X);this._fireEvent(V,W,Y.parentNode);}
;}
...
;this._fireEvent(bh,be,bc.target);if(bd.type==v||bd.type==G){var bi=new qx.event.type.dom.Pointer(N,bd,bc);this._fireEvent(bi,N,bd.target);if(this.__primaryIdentifier==bg.identifier){this.__primaryIdentifier=null;}
;}
;}
;}
...
;}
,_fireEvent:function(bw,bv,bx){bx=bx||bw.target;bv=bv||bw.type;var by;if(bv==z||bv==u||bv==M){by=new qx.event.type.dom.Pointer(qx.event.handler.PointerCore.POINTER_TO_GESTURE_MAPPING[bv],bw);qx.event.type.dom.Pointer.normalize(by);by.srcElement=bx;}
...
,stopMomentum:function(){if(this.__rollImpulseId){this.__stopMomentum=true;}
;}
,__handleRollImpulse:function(bh,bi,be,bf,bj){if((Math.abs(bi)<0.1&&Math.abs(bh)<0.1)||this.__stopMomentum){this.__stopMomentum=false;this.__rollImpulseId=null;return;}
;if(!bj){bj=1;var bg=2.8;bi=bi/bg;bh=bh/bg;}
;bj+=0.0006;bi=bi/bj;bh=bh/bj;this.__rollImpulseId=qx.bom.AnimationFrame.request(qx.lang.Function.bind(function(bk,bl,bm,bn,bo){this.__handleRollImpulse(bk,bl,bm,bn,bo);}
,this,bh,bi,be,bf,bj));bh=Math.round(bh*100)/100;bi=Math.round(bi*100)/100;be.delta={x:-bh,y:-bi};be.momentum=true;this._fireEvent(be,h,be.target||bf);}
,_calcAngle:function(){var bq=null;var br=null;for(var bp in this.__gesture){var bs=this.__gesture[bp];if(bq===null){bq=bs;}
else {br=bs;}
;}
;var x=bq.clientX-br.clientX;var y=bq.clientY-br.clientY;return (360+Math.atan2(y,x)*(180/Math.PI))%360;}
,_calcDistance:function(){var bt=null;var bu=null;for(var bw in this.__gesture){var bx=this.__gesture[bw];if(bt===null){bt=bx;}
else {bu=bx;}
;}
;var bv=Math.sqrt(Math.pow(bt.clientX-bu.clientX,2)+Math.pow(bt.clientY-bu.clientY,2));return bv;}
,_calcVelocity:function(bC,by,bz,bD,bA){var bB=0;if(bA==e){if(bD==D){bB=bC.velocityX;}
else {bB=bC.velocityY;}
;bB=bB/(new Date().getTime()-bC.lastEventTime);}
else {bB=(bz!==0)?by/bz:0;}
;return bB;}
,_isBelowTapMaxDistance:function(bF){var bE=this._getDeltaCoordinates(bF);if(!bE){return null;}
;var bG=qx.event.handler.GestureCore;return (Math.abs(bE.x)<=bG.TAP_MAX_DISTANCE[bF.pointerType]&&Math.abs(bE.y)<=bG.TAP_MAX_DISTANCE[bF.pointerType]);}
,__isBelowDoubleTapDistance:function(bH,bL,bM,bN,bO){var bK=qx.event.handler.GestureCore;var bI=Math.abs(bH-bM)<bK.TAP_MAX_DISTANCE[bO];var bJ=Math.abs(bL-bN)<bK.TAP_MAX_DISTANCE[bO];return bI&&bJ;}
,_getDeltaCoordinates:function(bR){var bS=this.__gesture[bR.pointerId];if(!bS){return null;}
;var bP=bR.clientX-bS.startX;var bQ=bR.clientY-bS.startY;var bT=D;if(Math.abs(bP/bQ)<1){bT=n;}
;return {"x":bP,"y":bQ,"axis":bT};}
,_fireEvent:function(bV,bX,bW){if(!this.__defaultTarget){return;}
;var bU;if(qx.core.Environment.get(c)){bU=new qx.event.type.dom.Custom(bX,bV,{bubbles:true,swipe:bV.swipe,scale:bV.scale,angle:bV.angle,delta:bV.delta,pointerType:bV.pointerType,momentum:bV.momentum});bW.dispatchEvent(bU);}
else if(this.__emitter){bU=new qx.event.type.dom.Custom(bX,bV,{target:this.__defaultTarget,currentTarget:this.__defaultTarget,srcElement:this.__defaultTarget,swipe:bV.swipe,scale:bV.scale,angle:bV.angle,delta:bV.delta,pointerType:bV.pointerType,momentum:bV.momentum});this.__emitter.emit(bX,bV);}
;}
,_onDblClick:function(bY){var ca=qx.bom.Event.getTarget(bY);this._fireEvent(bY,A,ca);this._fireEvent(bY,p,ca);}
,__getSwipeGesture:function(cd,ce){var ck=this.__gesture[cd.pointerId];var cg=qx.event.handler.GestureCore;var cj=this._getDeltaCoordinates(cd);var ch=new Date().getTime()-ck.startTime;var cl=(Math.abs(cj.x)>=Math.abs(cj.y))?D:n;var cb=cj[cl];var cc=cg.SWIPE_DIRECTION[cl][cb<0?0:1];var ci=(ch!==0)?cb/ch:0;var cf={startTime:ck.startTime,duration:ch,axis:cl,direction:cc,distance:cb,velocity:ci};return cf;}
,__fireTrack:function(cm,cn,co){cn.delta=this._getDeltaCoordinates(cn);this._fireEvent(cn,cm,cn.target||co);}
,_fireRoll:function(cq,cp,cr){if(cq.type===qx.core.Environment.get(g).type){cq.delta={x:qx.util.Wheel.getDelta(cq,D)*10,y:qx.util.Wheel.getDelta(cq,n)*10};cq.delta.axis=Math.abs(cq.delta.x/cq.delta.y)<1?n:D;cq.pointerType=j;}
else {var cs=this.__gesture[cq.pointerId];cq.delta={x:-cs.velocityX,y:-cs.velocityY,axis:Math.abs(cs.velocityX/cs.velocityY)<1?n:D};}
;this._fireEvent(cq,h,cq.target||cr);}
,__fireRotate:function(ct,cv){if(!ct.isPrimary){var cu=this._calcAngle();ct.angle=Math.round((cu-this.__initialAngle)%360);this._fireEvent(ct,k,this.__primaryTarget);}
;}
,__firePinch:function(cy,cz){if(!cy.isPrimary){var cw=this._calcDistance();var cx=cw/this.__initialDistance;cy.scale=(Math.round(cx*100)/100);this._fireEvent(cy,b,this.__primaryTarget);}
;}
,__fireLongTap:function(cA,cB){this._fireEvent(cA,f,cA.target||cB);var cC=this.__gesture[cA.pointerId];cC.longTapTimer=null;cC.isTap=false;}
,__stopLongTapTimer:function(cD){if(cD.longTapTimer){window.clearTimeout(cD.longTapTimer);cD.longTapTimer=null;}
;}
,isBelowTapMaxDistance:function(event){var cE=this._calcDelta(event);var cF=qx.event.handler.GestureCore;return (Math.abs(cE.x)<=cF.TAP_MAX_DISTANCE&&Math.abs(cE.y)<=cF.TAP_MAX_DISTANCE);}
,dispose:function(){for(var cG in this.__gesture){this.__stopLongTapTimer(cG);}
